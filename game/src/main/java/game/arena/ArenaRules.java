...
public final class ArenaRules {
    ...
    public static int starShardCost(int stars) {
        if (stars < 1 || stars >= 6) throw new IllegalArgumentException("星级非法");
        return 20 + (stars - 1) * 15;
    }

    public static long dungeonPower(int stage) {
        if (stage < 1 || stage > 12) throw new IllegalArgumentException("副本层数非法");
        return stage * 18000L;
    }

    public static long heroPower(long hp, long atk, long def, int rank, int stars, int skill) {
        positive(rank);
        positive(stars);
        positive(skill);
        long base = hp / 5 + atk * 8 + def * 5;
        return base + (rank - 1) * 900L + (stars - 1) * 2400L + (skill - 1) * 650L;
    }
