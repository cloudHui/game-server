	...
	public GameProto.AckTableSnapshot buildTableSnapshot(TableUser viewer) {
		...
		if (viewer.getSeated() == getOp().getCurrOpSeat() && b.getChoicesCount() == 0) {
			if (getTableState() == TableState.MJ_DISCARD) {
				...
				if (claim != null) {
					if (claim.isCanHu()) b.addChoices(GameProto.OpInfo.newBuilder().setChoice(ConstProto.Operation.MJ_HU));
					if (claim.isCanGang()) b.addChoices(GameProto.OpInfo.newBuilder().setChoice(ConstProto.Operation.MJ_GANG));
					if (claim.isCanPeng()) b.addChoices(GameProto.OpInfo.newBuilder().setChoice(ConstProto.Operation.MJ_PENG));
					if (claim.isCanChi()) b.addChoices(GameProto.OpInfo.newBuilder().setChoice(ConstProto.Operation.MJ_CHI));
