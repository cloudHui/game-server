...
public final class TractorRules {
    ...
    public static void sortHand(List<Card> cards, int levelRank, int trumpSuit) {
        ...
        cards.sort(Comparator.comparingInt(a -> handOrderKey(a, levelRank, trumpSuit)));
    }
    ...
    public static Combo analyze(List<Card> cards, int levelRank, int trumpSuit) {
        ...
        if (n == 2) {
            if (pairKey(sorted.get(0)) == pairKey(sorted.get(1))
        ...
        if (n >= 4 && n % 2 == 0) {
            return tryTractor(sorted, levelRank, trumpSuit, g0);
        }
        return null;
    }
    ...
    private static int pairKey(Card c) {
        isJoker(c);
        return c.getCardVal();
    }
    ...
    public static boolean isLegalFollow(List<Card> play, Combo lead, List<Card> hand,
        ...
        for (Card c : play) {
            if (suitGroup(c, levelRank, trumpSuit) != gLead) {
                sameGroup = false;
                break;
            }
        }
        ...
        if (!inLeadSuit.isEmpty()) {
            ...
            if (lead.type == ComboType.PAIR || lead.type == ComboType.TRACTOR) {
                ...
                if (havePairs >= needPairs) {
                    return parsed != null && parsed.type == lead.type
                            && (lead.type != ComboType.TRACTOR || parsed.tractorLen == lead.tractorLen);
                }
            }
            return true;
        }
        ...
        if (parsed != null && parsed.suitId == 0) {
            ...
            return lead.type != ComboType.TRACTOR || parsed.tractorLen == lead.tractorLen;
        }
    ...
    private static int countPairs(List<Card> cards, int levelRank, int trumpSuit) {
        ...
        int pairs = 0;
        for (long v : cnt.values()) pairs += (int) (v / 2);
        return pairs;
    }
