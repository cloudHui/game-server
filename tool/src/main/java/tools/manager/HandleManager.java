...
public class HandleManager {
    ...
    public static void sendMsg(int msgId, Message req, Sender handler, Parser parser, int sequence, int userId, boolean backError) {
        ...
        handler.sendMessageBackTcp(req, msgId, SEND_MESSAGE_TIME_OUT).whenComplete((msg, throwable) -> {
            try {
                if (throwable != null) {
                    LOGGER.error("消息发送异常 - 消息ID:0x{},用户ID:{},异常信息:{}", Integer.toHexString(msgId), userId, throwable.getMessage(), throwable);
                    return;
                }

                if (msg.getResult() != 0) {
                    LOGGER.warn("消息处理失败 - 消息ID:0x{},返回码:{},用户ID:{}", Integer.toHexString(msg.getMessageId()), msg.getResult(), userId);
