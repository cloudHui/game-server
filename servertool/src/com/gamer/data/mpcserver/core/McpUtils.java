...
public final class McpUtils {
    ...
    /** 按顺序读取第一个非空文本参数。 */
    public static String text(JsonNode params, String... keys) {
        for (String key : keys) {
    ...
    /** 读取必填文本参数。 */
    public static String requiredText(JsonNode params, String errorMessage, String... keys) {
        String value = text(params, keys);
        if (value == null) {
    ...
    /** 读取布尔参数，缺失时返回默认值。 */
    public static boolean bool(JsonNode params, String key, boolean defaultValue) {
        String value = text(params, key);
