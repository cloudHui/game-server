 ...
 *
 * <p>
 * tail 模式优化：使用 {@link RandomAccessFile} 从文件末尾向前扫描， 只读取需要的字节，避免读取整个文件（O(fileSize) → O(tail * avgLineLen)）。
 * </p>
 */
@Process(value = "read_text_file", description = "读取 UTF-8 文本文件。", required = {"path"},
    optional = {"head", "tail", "maxLines", "maxChars"})
public class ReadText implements CommandHandler {
    ...
    public CommandResult handle(CommandContext ctx, JsonNode params) throws Exception {
        ...
        int maxLines =
            normalizePositiveInt(McpUtils.intVal(params, "maxLines"), DEFAULT_MAX_LINES, MAX_LINES_UPPER_BOUND);
        int maxChars =
            normalizePositiveInt(McpUtils.intVal(params, "maxChars"), DEFAULT_MAX_CHARS, MAX_CHARS_UPPER_BOUND);
        if (head != null && head < 0) {
            head = null;
        }
        if (tail != null && tail < 0) {
     ...
     *
     * <p>
     * 算法：先估算偏移量（n * TAIL_AVG_LINE_BYTES），从该偏移向后读取并收集行； 若行数不足则向前再扩大块继续读，直到行数满足或读到文件头。 最终只返回末尾 n 行。
     * </p>
     */
