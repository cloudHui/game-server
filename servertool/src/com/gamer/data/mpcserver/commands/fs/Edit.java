 ...
 * </pre>
 */
@Process(value = "edit_file", description = "编辑文本文件。", readOnly = false, required = {"path", "edits"},
    optional = {"dryRun"})
public class Edit implements CommandHandler {
    @Override
    public CommandResult handle(CommandContext ctx, JsonNode params) throws Exception {
        ...
        if (dryRun) {
            ...
            McpServiceLog.cmdAndResult(ctx.log(), McpServiceLog.SERVICE_FS, "editFile path=" + path + " dryRun=true",
                dry);
