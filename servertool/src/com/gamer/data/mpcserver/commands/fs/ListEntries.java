 ...
 *
 * <p>
 * 使用 {@link DirectoryStream} 按需迭代，不再用 {@code listFiles()} 把所有条目加载进内存， 对包含大量文件的目录可提前退出，避免全量内存分配。
 * </p>
 */
@Process(value = "list_directory", description = "列出目录内容。", required = {"path"},
    optional = {"offset", "limit", "maxOutputChars"})
public class ListEntries implements CommandHandler {
    ...
    public CommandResult handle(CommandContext ctx, JsonNode params) throws Exception {
        ...
        int maxChars =
            normalizePositiveInt(McpUtils.intVal(params, "maxOutputChars"), DEFAULT_MAX_CHARS, MAX_MAX_CHARS);
