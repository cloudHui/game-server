 ...
 *
 * <p>
 * 匹配对象为“相对 path 的相对路径”，统一用 '/' 作为分隔符。
 * </p>
 */
@Process(value = "search_files", description = "递归搜索文件。", required = {"path", "pattern"},
    optional = {"excludePatterns", "maxDepth", "maxNodes", "maxResults", "maxOutputChars"})
public class Search implements CommandHandler {
    ...
    public CommandResult handle(CommandContext ctx, JsonNode params) throws Exception {
        ...
        int maxResults =
            normalizePositiveInt(McpUtils.intVal(params, "maxResults"), DEFAULT_MAX_RESULTS, MAX_MAX_RESULTS);
        ...
        int maxOutputChars = normalizePositiveInt(McpUtils.intVal(params, "maxOutputChars"), DEFAULT_MAX_OUTPUT_CHARS,
            MAX_OUTPUT_CHARS_UPPER_BOUND);
    ...
    private void appendSearchHeader(StringBuilder sb, int maxOutputChars, long elapsedMs, SearchState state) {
        String h = "SEARCH_MS=" + elapsedMs + " nodesVisited=" + state.nodesVisited + " maxResults=" + state.maxResults
            + " maxDepth=" + state.maxDepth + " maxNodes=" + state.maxNodes + " depthLimited=" + state.depthLimited
            + " resultLimited=" + state.resultLimited + " nodeLimited=" + state.nodeLimited;
