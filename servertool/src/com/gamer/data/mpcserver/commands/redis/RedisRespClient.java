 ...
 *
 * <p>
 * 避免引入 Jedis/lettuce 依赖：servertool/lib 当前不包含 redis 客户端 jar。
 * </p>
 *
 * <p>
 * 支持两种使用模式：
 ...
 * <li>会话模式：{@link #openSession()}，返回 Session 对象可复用同一个连接执行多条命令； Session 实现 AutoCloseable，但为兼容热更新约束不使用
 * try-with-resources， 需在 finally 中调用 {@link Session#close()}。</li>
 * </ul>
 */
public class RedisRespClient {
    ...
    /**
     * 一次性执行：内部新建连接、执行命令、关闭连接。 适用于单次 GET/SET/DEL 操作。
     */
