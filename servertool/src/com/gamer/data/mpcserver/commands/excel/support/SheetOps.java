...
public final class SheetOps {
    ...
    public static void saveWorkbook(Workbook wb, File file) throws Exception {
        ...
        File tmp = File.createTempFile("mcp", ".tmp", file.getParentFile());
        try {
            try (FileOutputStream fos = new FileOutputStream(tmp)) {
                wb.write(fos);
            }
            try {
                // 同目录优先原子替换；系统不支持时再降级为普通覆盖
     ...
     *
     * <p>
     * 不支持包含 sheetName 的复杂形式（如 Sheet1!A1）。
     * </p>
     */
    ...
    public static String renderRangeToPngBase64(Workbook wb, org.apache.poi.ss.usermodel.Sheet sheet, CellRange range,
        int maxRows, int maxCols, int maxCellTextLen) throws Exception {
    ...
    public static void applyStyleToRange(org.apache.poi.ss.usermodel.Sheet sheet, CellRange range, CellStyle style) {
