 ...
 * </ul>
 */
@Process(value = "excel_screen_capture", description = "将工作表区域渲染为 PNG。", required = {"fileAbsolutePath", "sheetName"},
    optional = {"range", "maxRows", "maxCols", "maxCellTextLen"})
public class CaptureSheet implements CommandHandler {
    ...
    public CommandResult handle(CommandContext ctx, JsonNode params) throws Exception {
        ...
        String base64 = SheetOps.renderRangeToPngBase64(wb, sheet, cr, mr, mc, mct);
        String out = "PNG_BASE64=" + base64;
        McpServiceLog.cmdAndResult(
            ctx.log(), McpServiceLog.SERVICE_EXCEL, "screenCapture file=" + file.getAbsolutePath() + " sheet="
                + sheetName + " range=" + (range == null ? "" : range.trim()) + " maxRows=" + mr + " maxCols=" + mc,
            out);
