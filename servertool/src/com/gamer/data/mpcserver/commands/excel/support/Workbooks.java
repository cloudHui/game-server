package com.gamer.data.mpcserver.commands.excel.support;
 ...
 *
 * <p>
 * 解决 ExcelReadCommand / ExcelDescribeSheetsCommand 每次都完整解析 Workbook 的性能问题。 同一文件在未修改的情况下只解析一次，后续调用直接复用缓存中的 Workbook 对象。
 * </p>
 *
 * <p>
 * 设计要点：
 ...
 * <li>线程安全：get/put 用 synchronized。Excel 工具跑在单线程池上，不会并发使用同一 Workbook。</li>
 * </ul>
 *
 * <p>
 * Workbook 不是线程安全的，禁止跨线程共用同一实例。
 * </p>
 */
public final class Workbooks {
    ...
    /** 全局单例。 */
    private static final Workbooks INSTANCE = new Workbooks();
    /**
     * 基于 accessOrder 的 LRU LinkedHashMap；超过 MAX_ENTRIES 时淘汰最久未使用的条目。 removeEldestEntry 需要关闭被淘汰 Workbook，所以在 put 前手动检查。
     */
    ...
    public static Workbooks getInstance() {
        return INSTANCE;
    }
     ...
     *
     * <p>
     * 若缓存命中且文件未修改，直接返回缓存对象；否则重新解析并更新缓存。
     * </p>
     *
     * @param file
     *            必须存在且是 .xls/.xlsx 文件
     ...
     *
     * <p>
     * 经 InputStream 解析到内存，不绑定原 File；缓存淘汰关闭时 revert OPC 包，不 save、不刷新 mtime。
     * </p>
     *
     * @param file
     *            必须存在且是 .xls/.xlsx 文件
    ...
    /**
     * 在同一 Workbook 实例上完成写盘成功后调用：把条目中记录的时间戳更新为当前文件修改时间， 避免紧接着的 {@link #get(File)} 因 mtime 漂移而关闭并重建整本 Workbook。
     *
     * @param file
     *            已保存的 Excel 文件
     * @param wb
     *            必须与 {@link #get(File)} 返回的为同一引用
     */
    ...
    /**
     * 编辑失败时移除并关闭指定的可写 Workbook，避免未保存修改污染后续命令。
     *
     * @param file
     *            Excel 文件
     * @param wb
     *            本次编辑使用的 Workbook
     */
    public synchronized void discard(File file, Workbook wb) {
        if (file == null || wb == null) {
            return;
        }
        String key = cacheKey(file);
        Entry entry = cache.get(key);
        if (entry == null || entry.get() != wb) {
            return;
        }
        cache.remove(key);
        closeEntry(entry);
