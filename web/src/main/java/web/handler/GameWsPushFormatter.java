...
final class GameWsPushFormatter {
	...
	static Map<String, Object> formatSnapshot(GameProto.AckTableSnapshot n) {
		Map<String, Object> m = new HashMap<>();
		m.put("tableId", n.getTableId()); m.put("gameType", n.getGameType());
		m.put("version", n.getVersion()); m.put("round", n.getRound());
		m.put("state", n.getState()); m.put("stateStart", n.getStateStart());
		m.put("stateDuration", n.getStateDuration()); m.put("opSeat", n.getOpSeat());
		m.put("choices", formatOpChoices(n.getChoicesList()));
		List<Map<String, Object>> players = new ArrayList<>();
		for (GameProto.SnapshotPlayer p : n.getPlayersList()) {
			Map<String, Object> x = new HashMap<>();
			x.put("roleId", p.getRoleId()); x.put("position", p.getSeat());
			x.put("nickName", p.getNick().toStringUtf8()); x.put("online", p.getOnline());
			x.put("totalScore", p.getTotalScore()); x.put("cardCount", p.getCardCount());
		...
		for (GameProto.SnapshotDiscard d : n.getDiscardsList()) {
			Map<String, Object> x = new HashMap<>();
			x.put("seat", d.getSeat()); x.put("tileId", d.getTileId()); x.put("sequence", d.getSequence());
			discards.add(x);
		}
		...
		for (GameProto.SnapshotExposed e : n.getExposedList()) {
			Map<String, Object> x = new HashMap<>();
			x.put("seat", e.getSeat()); x.put("type", e.getType().toStringUtf8());
			x.put("tileIds", new ArrayList<>(e.getTileIdsList())); exposed.add(x);
		}
		m.put("exposed", exposed);
		m.put("drawnTile", n.getDrawnTile()); m.put("pendingDiscardTile", n.getPendingDiscardTile());
		m.put("pendingDiscardSeat", n.getPendingDiscardSeat()); m.put("wallLeft", n.getWallLeft());
		m.put("laiziTile", n.getLaiziTile()); m.put("laiziFlipTile", n.getLaiziFlipTile());
		m.put("dealerSeat", n.getDealerSeat()); m.put("bottomCards", new ArrayList<>(n.getBottomCardsList()));
		List<Integer> lastCards = new ArrayList<>();
		for (GameProto.Card c : n.getLastCards().getCardsList()) lastCards.add(c.getValue());
		m.put("lastCards", lastCards); m.put("lastPlaySeat", n.getLastPlaySeat());
		m.put("passSeats", new ArrayList<>(n.getPassSeatsList())); m.put("landlordSeat", n.getLandlordSeat());
		m.put("baseScore", n.getBaseScore()); m.put("robMultiplier", n.getRobMultiplier());
		m.put("bombMultiplier", n.getBombMultiplier()); m.put("currentMultiplier", n.getCurrentMultiplier());
		return m;
	}
	...
	private static Map<String, Object> formatNotRoundResult(GameProto.NotRoundResult n) {
		...
		for (GameProto.SeatScore s : n.getTotalScoresList()) {
			Map<String, Object> sc = new HashMap<>();
			sc.put("seat", s.getSeat()); sc.put("score", s.getScore());
			totals.add(sc);
		}
