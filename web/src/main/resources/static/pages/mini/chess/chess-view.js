(function (w) {
    ...
    function create(canvas, getBoard, getSelected, names) {
        ...
        function layout() {
            ...
            px = width * .08;
            py = height * .06;
            cw = (width - px * 2) / (cols - 1);
            ch = (height - py * 2) / (rows - 1);
            cacheBoard();
            cachePieces();
            draw();
        }
        ...
        function cachePieces() {
            ...
            Object.keys(names).forEach(function (piece) {
                sprites[piece] = CanvasBuffer.createSprite(size, function (ctx, width) {
                    ...
                    ctx.fillStyle = red ? '#fff5e6' : '#2a2a2a';
                    ctx.fill();
                    ctx.strokeStyle = red ? '#c41e2a' : '#111';
                    ctx.lineWidth = 2;
                    ctx.stroke();
        ...
        function drawSelection(ctx, board, selected) {
            ...
            ctx.beginPath();
            ctx.arc(point[0], point[1], Math.min(cw, ch) * .42, 0, Math.PI * 2);
