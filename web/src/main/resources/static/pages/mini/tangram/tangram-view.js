...
var TangramView = (function () {
  ...
  function polygonsOverlap(a, b) {
    function axisSeparates(poly, other) {
      for (var i = 0; i < poly.length; i++) {
        ...
        ax /= axisLength;
        ay /= axisLength;
        var amin = Infinity; var amax = -Infinity;
        var bmin = Infinity; var bmax = -Infinity;
        for (var j = 0; j < poly.length; j++) { var ap = poly[j][0] * ax + poly[j][1] * ay; amin = Math.min(amin, ap); amax = Math.max(amax, ap); }
        for (var k = 0; k < other.length; k++) { var bp = other[k][0] * ax + other[k][1] * ay; bmin = Math.min(bmin, bp); bmax = Math.max(bmax, bp); }
  ...
  return {
    worldPts: worldPts,
    aabbOf: aabbOf,
    polygonsOverlap: function (a, b) { return polygonsOverlap(worldPts(a), worldPts(b)); },
    centroid: centroid,
    render: render
  };
})();
