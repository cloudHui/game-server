...
(function () {
  ...
  function updateStatus() {
    ...
    if (done === state.pieces.length) {
      ...
      if (!completed) {
        completed = true;
        MiniResult.show({pattern: '「' + levels[state.level].name + '」', elapsed: formatElapsed(Date.now() - startedAt), onNext: function(){ document.getElementById('next').click(); }});
      }
    } else {
  ...
  function formatElapsed(ms) { var sec = Math.max(0, Math.round(ms / 1000)); return Math.floor(sec / 60) + '分' + (sec % 60) + '秒'; }
