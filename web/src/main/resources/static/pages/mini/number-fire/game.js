...
(function (w) {
  ...
  function Game() {
    ...
    this.plane = { x: 360, y: 460, width: 80, height: 60, targetX: 360, isMoving: false, pending: false, target: null, rotation: 0 };
    this.targets = [];
    this.queue = [];
    this.bullets = [];
    this.explosions = [];
  ...
  Game.prototype.bindUi = function () {
    var self = this;
    document.getElementById('btnRetry').addEventListener('click', function () { self.startLevel(self.levelIndex); });
    document.getElementById('btnNext').addEventListener('click', function () { self.goNextLevel(); });
    document.getElementById('btnLobby').addEventListener('click', function () { self.endToLobby(); });
    document.getElementById('btnBackLobby').addEventListener('click', function () { self.endToLobby(); });
    ...
    if (w.MiniFirePad) {
      w.MiniFirePad.mountDigits(document.getElementById('touchPad'), function (key) {
        self.onKey({ key: key, preventDefault: function () {} });
      });
    }
  };
  ...
  Game.prototype.tryMatchInput = function () {
    ...
    if (!buf) return;
    var target = this.targets.find(function (t) { return String(t.value) === buf; });
    if (!target) {
      var prefix = this.targets.some(function (t) { return String(t.value).indexOf(buf) === 0; });
  ...
  Game.prototype.loop = function () {
    ...
    this.draw();
    if (this.running && !this.ended) requestAnimationFrame(function () { self.loop(); });
  };

  w.onload = function () { new Game(); };
})(window);
