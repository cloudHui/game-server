(function (w) {
    'use strict';
    var pollTimer, watchGeneration = 0, select = document.getElementById('robotMatchRoom');
    ...
    function watchTable(tableId, attempt, generation) {
        ...
        Admin.get('/replays?page=1&size=100').then(function (data) {
            ...
            pollTimer=setTimeout(function(){watchTable(tableId,(attempt||0)+1,generation)},1000);
        }).catch(function(){if(generation===watchGeneration)pollTimer=setTimeout(function(){watchTable(tableId,(attempt||0)+1,generation)},1500)});
    }
    ...
    w.loadTables=load;w.watchLiveTable=function(tableId){Admin.msg('tablesMsg','正在进入桌 '+tableId+'…');ReplayPlayer.waiting(tableId);watchGeneration++;watchTable(tableId,0,watchGeneration)};
    ...
    w.startRobotMatch = function () {
        ...
        Admin.post('/robot-matches', payload).then(function (data) {
            ...
            Admin.msg('robotMatchMsg', '测试已启动：桌号 ' + data.tableId + '，正在进入实时对局。', true);load();ReplayPlayer.waiting(data.tableId);watchGeneration++;watchTable(data.tableId,0,watchGeneration);
        }).catch(function () { button.disabled = false; Admin.msg('robotMatchMsg', '网络错误', false); });
    };
    w.changeRobotRules();
    document.addEventListener('replay-player-close',function(){watchGeneration++;clearTimeout(pollTimer)});
})(window);
