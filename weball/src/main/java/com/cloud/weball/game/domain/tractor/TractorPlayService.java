package com.cloud.weball.game.domain.tractor;
...
import com.cloud.weball.game.domain.replay.PokerReplayRecorder;
import com.cloud.weball.game.domain.tractor.ai.TractorSimpleAi;
...
public final class TractorPlayService {
        ...
        if (lead == null) {
            ...
            if (parsed == null) {
                ...
                if (!throwCanStand(table, user, selected, level, trump)) {
                    ...
                    selected = Collections.singletonList(smallest);
                    ids = Collections.singletonList(smallest.getId());
    ...
    private static void afterPlay(TractorTable table, TableUser user, List<Card> selected) {
        ...
        GameProto.CardInfo.Builder ci = GameProto.CardInfo.newBuilder().setType(ConstProto.CardType.SINGLE);
