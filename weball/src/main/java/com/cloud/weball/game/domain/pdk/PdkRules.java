...
import com.cloud.weball.game.domain.ddz.DdzHand;
 ...
 * 跑得快牌型识别与比大小。
 * <p>
 * 支持：单、对、三不带、三带一、三带二、顺≥5、连对≥2、炸弹(四张)。2 不进顺/连对。
 * 快速房默认三带一与三带二均可出，跟牌须同型（带一同带一、带二同带二）。
 */
public final class PdkRules {
	...
	public static List<DdzHand> enumerateLeadHands(List<Card> cards) {
		...
		for (Map.Entry<Integer, List<Card>> e : groups.entrySet()) {
			...
			for (Map.Entry<Integer, List<Card>> p : groups.entrySet()) {
				if (p.getKey() == tripleVal) continue;
				List<Card> wing = p.getValue();
				if (wing.isEmpty()) continue;
				// 三带一：任意一张单牌
				List<Card> t1 = new ArrayList<>(4);
				t1.addAll(e.getValue().subList(0, 3));
				t1.add(wing.get(0));
				addHand(result, seen, handOf(ConstProto.CardType.TRIPLE_ONE, t1, tripleVal, 0, false));
				if (wing.size() >= 2) {
					List<Card> t2 = new ArrayList<>(5);
					t2.addAll(e.getValue().subList(0, 3));
					t2.addAll(wing.subList(0, 2));
	...
	private static void collectTripleOnes(Map<Integer, List<Card>> groups, int minimum, List<DdzHand> out) {
		for (int triple = minimum + 1; triple <= CardConst.ER_VAL; triple++) {
			...
			for (Map.Entry<Integer, List<Card>> entry : groups.entrySet()) {
				if (entry.getKey() == triple || entry.getValue().isEmpty()) continue;
				List<Card> result = new ArrayList<>(4);
				result.addAll(tripleCards.subList(0, 3));
				result.add(entry.getValue().get(0));
				out.add(handOf(ConstProto.CardType.TRIPLE_ONE, result, triple, 0, false));
			}
		}
	}

	private static Optional<List<Card>> findTripleOne(Map<Integer, List<Card>> groups, int minimum) {
		for (int triple = minimum + 1; triple <= CardConst.ER_VAL; triple++) {
			List<Card> tripleCards = groups.get(triple);
			if (tripleCards == null || tripleCards.size() < 3) continue;
			for (Map.Entry<Integer, List<Card>> entry : groups.entrySet()) {
	...
	private static Optional<DdzHand> tryTripleOne(Map<Integer, Long> cnt, List<Card> sorted) {
		if (sorted.size() != 4) return Optional.empty();
		Integer triple = null, single = null;
		for (Map.Entry<Integer, Long> e : cnt.entrySet()) {
			if (e.getValue() == 3L) triple = e.getKey();
			else if (e.getValue() == 1L) single = e.getKey();
			else return Optional.empty();
		}
		if (triple == null || single == null) return Optional.empty();
		return Optional.of(new DdzHand(ConstProto.CardType.TRIPLE_ONE, sorted, false, false, triple, 0));
	}
