...
import com.cloud.game.manager.table.mj.card.MjTilePool;
...
public class MjDrawService {
    ...
    public static int drawTile(MjTable table) {
        ...
        logger.info("麻将摸牌, table: {}, userId: {}, seat: {}, tile: {}, hand: {} -> {}, exposed: {}, 剩余: {}",
                table.getTableId(), user.getUserId(), seat, tileId, beforeCount, user.getCards().size(),
                ctx.getExposedSets(seat).size(), tilePool.remaining());
        return tileId;
    }
    ...
    public static void autoDiscard(MjTable table) {
        ...
        if (!ctx.isTileDrawn() && !ctx.isDiscardAfterClaim()) {
            logger.error("麻将自动出牌被拦截: 本回合未摸牌且非碰吃接牌, table: {}, seat: {}, state: {}",
                    table.getTableId(), seat, table.getTableState());
            return;
        }
        ...
        logger.info("麻将超时自动出牌, table: {}, userId: {}, seat: {}, tile: {}, hand: {} -> {}, exposed: {}",
                table.getTableId(), user.getUserId(), seat, tileId, beforeCount, user.getCards().size(),
                ctx.getExposedSets(seat).size());
    }
