package com.cloud.game.manager.table.tractor.ai;

import com.cloud.game.manager.table.TableUser;
import com.cloud.game.manager.table.cards.Card;
import com.cloud.game.manager.table.tractor.TractorRules;
import com.cloud.game.manager.table.tractor.TractorTable;
import com.cloud.game.manager.table.tractor.TractorTableContext;
...
public final class TractorSimpleAi {
	...
	private static List<Card> pickRequiredPairs(
		...
		// 有足够对子但构不成连对拖拉机：仍须出同色 needPairs 个对子，避免卡死
		if (candidates.isEmpty() && pairs.size() >= needPairs) {
			List<Card> forced = new ArrayList<>(needPairs * 2);
			for (int i = 0; i < needPairs; i++) forced.addAll(pairs.get(i));
			return forced;
		}
		if (candidates.isEmpty()) {
			// 对子不够：先出全部同色对，再用同色单牌补张
			return pickPairsThenPad(pairs, lead.cards.size(),
					filterSuit(hand, lead.suitId, level, trump), level, trump,
					partnerWinning, lastToPlay);
		}
	...
	/** 对子不够跟拖拉机时：先垫全部同色对，再用同色单牌补足张数。 */
	private static List<Card> pickPairsThenPad(
            List<List<Card>> pairs, int need, List<Card> sameSuit, int level, int trump,
            boolean partnerWinning, boolean lastToPlay) {
		List<Card> out = new ArrayList<>(need);
		for (List<Card> pair : pairs) {
			if (out.size() + 2 > need) break;
			out.addAll(pair);
		}
		if (out.size() >= need) return out;
		List<Card> left = new ArrayList<>(sameSuit);
		left.removeAll(out);
		out.addAll(pickPad(left, need - out.size(), level, trump, partnerWinning, lastToPlay));
		return out;
	}
