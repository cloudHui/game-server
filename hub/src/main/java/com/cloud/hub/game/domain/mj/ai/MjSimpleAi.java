package com.cloud.hub.game.domain.mj.ai;
...
import com.cloud.hub.game.domain.mj.MjClaimInfo;
import com.cloud.hub.game.domain.mj.MjTable;
import com.cloud.hub.game.domain.mj.card.MjConst;
...
public final class MjSimpleAi {
    ...
    public static int decideDiscard(MjTable table, TableUser user, int drawnTile) {
        int aiLevel = table.getMjContext().getAiLevel();
        int visionLevel = AiVision.effectiveVisionLevel(
                table.getMjContext().getVisionLevel(), aiLevel);
        MjVision vision = new MjVision(table, user, visionLevel, aiLevel);
        aiLevel = vision.getAiLevel();
        ...
        if (aiLevel >= MjVision.AI_MASTER) {
            return MjMasterAi.decideDiscard(hand, vision, new AiSearchBudget(80, 1800));
        }
        // Level 1/2: 基于向听数选牌
        return decideByEfficiency(hand, vision);
    }

    static List<Integer> allTileTypes() {
        List<Integer> tiles = new ArrayList<>(34);
        for (int suit = 1; suit <= 5; suit++) {
            int max = suit <= 3 ? 9 : (suit == 4 ? 4 : 3);
            for (int value = 1; value <= max; value++) tiles.add(MjConst.encode(suit, value));
        }
        return tiles;
    }
     ...
     * 基于向听数 + 牌效率的弃牌选择
     */
    static int decideByEfficiency(List<Card> hand, MjVision vision) {
    ...
    public static GameProto.OpInfo decideClaim(MjTable table, TableUser user, MjClaimInfo claimInfo) {
        int configuredLevel = table.getMjContext().getAiLevel();
        int visionLevel = AiVision.effectiveVisionLevel(
                table.getMjContext().getVisionLevel(), configuredLevel);
        MjVision vision = new MjVision(table, user, visionLevel, configuredLevel);
        ...
        if (claimInfo.isCanChi()) {
            int[] combo = pickChi(user, claimInfo.getChiCombos());
            if (combo != null) return chi(combo);
        }
    ...
    private static boolean shouldGang(MjTable table, TableUser user, MjVision vision, int gangTileId) {
        ...
        int shantenAfter = calcShanten(counts) - 2;
     ...
     * 是否应该吃
     */
    static int[] pickChi(TableUser user, List<int[]> combos) {
        if (combos == null || combos.isEmpty()) return null;
        int[] counts = handToCounts(new ArrayList<>(user.getCards()));
        int shantenBefore = calcShanten(counts);
        int[] best = null;
        int bestShanten = Integer.MAX_VALUE;
        for (int[] combo : combos) {
            if (!removeCombo(counts, combo)) continue;
            int shantenAfter = calcShanten(counts) - 2;
            restoreCombo(counts, combo);
            if (shantenAfter < bestShanten) {
                bestShanten = shantenAfter;
                best = combo;
            }
        }
        return bestShanten < shantenBefore ? best : null;
    }

    private static boolean removeCombo(int[] counts, int[] combo) {
        for (int i = 0; i < combo.length; i++) {
            int tileId = combo[i];
            if (tileId < 0 || tileId >= counts.length || counts[tileId] == 0) {
                for (int restored = 0; restored < i; restored++) counts[combo[restored]]++;
                return false;
            }
            counts[tileId]--;
        }
    ...
    private static void restoreCombo(int[] counts, int[] combo) {
        for (int tileId : combo) counts[tileId]++;
    }
     ...
     */
    static int[] handToCounts(List<Card> hand) {
     ...
     * @return 有效进张总数
     */
    static int countEffectiveDraws(int[] counts, int laiZi) {
