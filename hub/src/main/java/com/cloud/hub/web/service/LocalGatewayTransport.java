...
public final class LocalGatewayTransport implements GatewayTransport {
    ...
    private static CompletableFuture<TCPMessage> failedTcp(String reason) {
        return failed(reason);
    }

    private static CompletableFuture<LobbyProto.AckJoinRoomTable> failedJoin(String reason) {
        return failed(reason);
    }
    ...
    private static final class LocalSender implements Sender {
        ...
        public void sendMessage(int msgId, Message msg, int sequence) {
            complete(msgId, 0, 0, msg, sequence);
        }
