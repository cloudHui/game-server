...
public class ArenaPillService {
    ...
    public void consume(Connection c, long uid, int tier) throws SQLException {
        int t1, t2, t3, t4;
        try (PreparedStatement p = c.prepareStatement("SELECT pills_tier1, pills_tier2, pills_tier3, pills_tier4 FROM arena_player WHERE user_id=?")) {
            p.setLong(1, uid);
        ...
        if (!ArenaRules.canConsumePill(tier, t1, t2, t3, t4)) {
            throw new IllegalArgumentException("该阶丹药已达服用上限或总数已达 50 颗封顶");
        }
        try (PreparedStatement p = c.prepareStatement("UPDATE arena_player SET pills_tier" + tier + "=pills_tier" + tier + "+1 WHERE user_id=?")) {
            p.setLong(1, uid);
            p.executeUpdate();
        }
    }
    ...
    public void buy(Connection c, long uid) throws SQLException {
        ...
        if (!ArenaRules.canConsumePill(4, t1, t2, t3, t4)) {
            throw new IllegalArgumentException("四阶极品神丹已达上限（最多15颗）或总丹药已满 50 颗");
        }
        dao.spend(c, uid, "fate", 8);
        try (PreparedStatement p = c.prepareStatement("UPDATE arena_player SET pills_tier4=pills_tier4+1 WHERE user_id=?")) {
            p.setLong(1, uid);
            p.executeUpdate();
        }
    }
