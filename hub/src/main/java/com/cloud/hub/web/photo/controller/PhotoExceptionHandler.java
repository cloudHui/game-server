...
public class PhotoExceptionHandler {
    ...
    ResponseEntity<Map<String, Object>> photo(PhotoException e) {
        return response(e.getStatus(), e.getMessage());
    }

    @ExceptionHandler(MaxUploadSizeExceededException.class)
    ResponseEntity<Map<String, Object>> large() {
        return response(413, "上传请求超过大小限制");
    }

    @ExceptionHandler(Exception.class)
    ResponseEntity<Map<String, Object>> other(Exception e) {
        log.error("图片库请求失败", e);
        return response(500, "图片库处理失败");
    }

    private ResponseEntity<Map<String, Object>> response(int status, String msg) {
