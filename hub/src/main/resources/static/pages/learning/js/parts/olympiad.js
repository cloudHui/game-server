(function () {
    ...
    LearningRegister({
        ...
        computed: {
            currentOlympiad() {
                return this.olympiadQuestions[this.olympiadIndex] || {};
            },
            olympiadProgress() {
                return this.olympiadQuestions.length ? Math.round((this.olympiadIndex + 1) / this.olympiadQuestions.length * 100) : 0;
            }
        },
        methods: {
            selectOlympiadTopic(topic) {
                this.olympiadTopic = topic;
            ...
            async startOlympiad() {
                ...
                this.olympiadIndex = 0;
                this.olympiadAnswer = '';
                this.olympiadCorrect = 0;
                this.olympiadHintIndex = 0;
                this.olympiadHintsUsed = 0;
                this.olympiadResult = null;
                this.olympiadFinished = false;
                this.olympiadStartedAt = Date.now();
            ...
            showOlympiadHint() {
                ...
                if (this.olympiadHintIndex < count) {
                    this.olympiadHintIndex++;
                    this.olympiadHintsUsed++;
                }
            },
            async submitOlympiad() {
                ...
                this.olympiadResult = {
                    correct,
                    text: correct ? '答对了！你找到了正确方法。' : `答案是 ${q.answer}${q.unit}`
                };
            ...
            async saveOlympiadMistake(q, answer) {
                try {
                    await this.addMistake({
                        subject: '数学',
                        module: '奥数·' + this.olympiadTopic.name,
                        question: q.prompt,
                        userAnswer: String(answer),
                        correctAnswer: String(q.answer) + q.unit,
                        errorType: '方法或计算错误'
                    });
                } catch (error) {
            ...
            async nextOlympiad() {
                if (this.olympiadIndex >= this.olympiadQuestions.length - 1) {
                    await this.finishOlympiad();
                    return;
                }
                this.olympiadIndex++;
                this.olympiadAnswer = '';
            ...
            async finishOlympiad() {
                ...
                try {
                    await this.saveRecord('数学', '奥数·' + this.olympiadTopic.name, 5, this.olympiadCorrect,
                        {
                            topic: this.olympiadTopic.id,
                            level: this.olympiadLevel,
                            hintsUsed: this.olympiadHintsUsed
                        }, seconds);
                    await this.loadDashboard();
                    this.olympiadQuestions = [];
                    this.olympiadFinished = true;
                    this.showToast('本次训练已保存');
                } catch (error) {
                    this.showToast(error.message);
                } finally {
                    this.olympiadSaving = false;
                }
            },
            leaveOlympiad() {
                this.olympiadTopic = null;
                this.olympiadQuestions = [];
                this.olympiadResult = null;
                this.olympiadFinished = false;
            }
