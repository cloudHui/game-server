(function () {
    ...
    LearningRegister({
        ...
        methods: {
            async loadWords() {
                try {
                    this.words = await this.api('words?stage=' + encodeURIComponent(this.selectedStage));
                    if (!this.words.length && this.selectedStage !== '幼小衔接') {
            ...
            async setChineseMode(mode) {
                ...
                this.dictationRevealed = false;
                if (mode === 'learn') await nextTick(() => this.initCanvas());
                this.sendHeartbeat();
            },
            speak(text) {
                if (!('speechSynthesis' in window)) {
                    this.showToast('当前浏览器不支持语音播报');
                    return;
                }
            ...
            initCanvas() {
                ...
                canvas.onpointerdown = e => {
                    ...
                    this.canvasLast = point(e);
                    ctx.beginPath();
                    ctx.arc(this.canvasLast.x, this.canvasLast.y, ctx.lineWidth / 2, 0, Math.PI * 2);
                ...
                canvas.onpointermove = e => {
                    ...
                    const p = point(e);
                    ctx.beginPath();
                    ctx.moveTo(this.canvasLast.x, this.canvasLast.y);
                    ctx.lineTo(p.x, p.y);
                    ctx.stroke();
                    this.canvasLast = p;
                };
            ...
            clearCanvas() {
                const c = this.$refs.writingCanvas;
                if (c) c.getContext('2d').clearRect(0, 0, c.width, c.height);
            },
            async recordWord(correct) {
                if (!this.selectedWord) return;
                try {
                    await this.saveRecord('语文', '认字', 1, correct ? 1 : 0, {
                        wordId: this.selectedWord.id,
                        character: this.selectedWord.character
                    });
                    if (!correct) await this.addMistake({
                        subject: '语文',
                        module: '认字',
                        question: '认读汉字：' + this.selectedWord.character,
                        userAnswer: '不认识',
                        correctAnswer: this.selectedWord.pinyin + '；' + this.selectedWord.words,
                        errorType: '不认识'
                    });
            ...
            async startDictation() {
                if (!this.words.length) await this.loadWords();
                this.dictationWord = this.words[Math.floor(Math.random() * this.words.length)];
                this.dictationRevealed = false;
                await nextTick(() => this.initCanvas());
                this.speak(this.dictationWord.character);
            },
            async finishDictation(correct) {
                const word = this.dictationWord;
                try {
                    await this.saveRecord('语文', '听写', 1, correct ? 1 : 0, {
                        wordId: word.id,
                    ...
                    if (!correct) await this.addMistake({
                        subject: '语文',
                        module: '听写',
                        question: '听写：' + word.pinyin,
                        userAnswer: '书写错误',
                        correctAnswer: word.character,
                        errorType: '书写错误'
                    });
                    this.showToast(correct ? '听写正确！' : '已加入错题库');
                    this.dictationWord = null;
                    setTimeout(() => this.startDictation(), 300);
                } catch (error) {
                    this.showToast(error.message);
                }
            }
